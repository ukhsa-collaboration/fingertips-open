...
namespace PholioVisualisation.ExportTest
{
    ...
    public class LookUpManagerTest
    {
        ...
        public void TestLookUpMethods()
        {
            ...
            Assert.AreEqual("Aggregated from all known lower geography values", 
                lookUpManager.GetValueNoteText(ValueNoteIds.ValueAggregatedFromAllKnownGeographyValues));

            // Assert area look ups 
            Assert.IsTrue(lookUpManager.GetAreaTypeName(AreaCodes.Ccg_Barnet).Contains("CCG"));
            ...
            Assert.AreEqual("Black / African / Caribbean / Black British", 
                lookUpManager.GetCategoryName(CategoryTypeIds.EthnicGroups5, CategoryIds.EthnicityBlack));
        }
    }
}
