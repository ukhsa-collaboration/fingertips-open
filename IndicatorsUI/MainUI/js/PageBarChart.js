...
function addBarChartTemplate() {
    templates.add('indicators',
		'<div id="indicator-details-no-data" class="no-indicator-data">No Data</div>' +
		...
		'{{#hasRecentTrends}} <th><div class="center">Recent<br>Trend</div></th> {{/hasRecentTrends}}' +
		...
		'<th class="bar">&nbsp;</th><th title="Lower confidence interval"><div class="center">95%<br>Lower CI</div></th>' +
		'<th title="Upper confidence interval"><div class="center">95%<br>Upper CI</div></th></tr></thead>' +
...
function addBarChartOptions() {
    ...
    var parentAreaOption = FT.model.isNearestNeighbours()
        ? areaHash[FT.model.areaCode].Name + ' & nearest neighbours'
        : 'All in ' + getParentArea().Name;

    // Areas to display options
    var options = [parentAreaOption];
    if (FT.model.areaTypeId !== AreaTypeIds.Practice) {
        options.push('All in England');
    }
...
function getDataSortedByArea(dataList, areas) {
    var sortedData = [];
    for (var i in areas) {
        var data = getDataFromAreaCode(dataList, areas[i].Code);
